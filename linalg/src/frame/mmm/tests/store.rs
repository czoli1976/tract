...
/// while accumulating in f32), so this exercises the `add_unicast` load path
/// for that dtype. Without a dtype-aware load an f16 operand read as f32 yields
/// garbage that saturates, so this catches the class of bug where `add_unicast`
/// ignores `item_size`.
    ...
    usize: AsPrimitive<TC> + AsPrimitive<TI>,
{
    ...
    // garbage in the unread cells of the strided layouts.
    let mut operand = vec![TC::max_value(); operand_size];
    for r in 0..mr {
        for c in 0..nr {
            operand[r * row_stride + c * col_stride] = (1 + c + r * nr).as_();
        }
    }
    ...
    let err = ker.kernel(&non_linear);
    assert_eq!(err, 0);
    let expected: Vec<TI> = (0..mr * nr).map(|i| (1 + i).as_()).collect();
