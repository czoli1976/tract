// M5-GPU (Neural Accelerator) fused SDPA kernel, ported from Apple MLX
// (https://github.com/ml-explore/mlx) @ 337f736a9115d1db1384015cd23aa0d6eb76917b,
// MIT License, Copyright (c) 2023-2025 Apple Inc.
//
// Uses Metal 4 tensor ops (MetalPerformancePrimitives); requires macOS 26+ to
// COMPILE and runs on any Apple-silicon GPU (pre-M5 via the ALU fallback, M5+
// on the neural accelerators). Kept as a separate library from mlx_sdpa.metal
// so it is only compiled when the runtime gate probes successfully.
// Mechanical flattening of the MLX include closure, same rules as
// mlx_sdpa.metal; resync by re-flattening, do not hand-edit the sections.
//
// clang-format off
...
// ===== instantiations (adapted from mlx steel_attention_nax.metal: f32/f16,
// ===== bq64/bk32 only — the shapes the host dispatch requests) =====

using namespace metal;

#define instantiate_attn_nax(tname, dtype, bq, bk, bd, wm, wn, mname, mtype) \
  instantiate_kernel(                                                        \
      "steel_attention_" #tname "_bq" #bq "_bk" #bk "_bd" #bd                \
      "_wm" #wm "_wn" #wn "_mask" #mname,                                    \
  attention_nax, dtype, bq, bk, bd, wm, wn, mtype, float)

instantiate_attn_nax(float16, half, 64, 32, 128, 4, 1, float16, half);
instantiate_attn_nax(float16, half, 64, 32,  64, 4, 1, float16, half);
instantiate_attn_nax(float32, float, 64, 32, 128, 4, 1, float32, float);
instantiate_attn_nax(float32, float, 64, 32,  64, 4, 1, float32, float);
// clang-format on
