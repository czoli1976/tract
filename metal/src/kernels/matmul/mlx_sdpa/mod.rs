...
pub fn dispatch_mlx_sdpa(
    ...
    if vector_ok {
        ...
        dispatch_steel_attention(stream, dt, scale, is_causal, shape6, q, k, v, mask, out, nax)
    }
}
...
/// mlx-style parse of the GPU architecture name into (generation, is_phone):
/// "applegpu_g16s" -> (16, false), "applegpu_g17p" -> (17, true).
fn device_architecture_gen() -> Option<(u32, bool)> {
    let name = device_arch::name()?;
    let generation = name
        .chars()
...
fn device_is_m5_class() -> bool {
    ...
    let Some((generation, phone)) = device_architecture_gen() else { return false };
